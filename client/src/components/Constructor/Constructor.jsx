            ...
            <form onSubmit={submitAction}>
                ...
                {language === "java" ?
                    <div>
                        ...
                        <Framework frameworks={{fw1: "junit", titleFw1: "JUnit", fw2: "testng", titleFw2: "TestNG"}}
                            onChange={setFrameworkFunction_J}/>
                        <LibraryPattern sendArray={setArrayFunctionLibrary_J} isLangChanged={isLangChanged}
                                        data={dataLibrary.java} header={{header: "Add library"}}/>
                        <LibraryPattern sendArray={setArrayFunctionPattern_J} isLangChanged={isLangChanged}
                                        data={dataPattern.java} header={{header: "Add pattern"}}/>
                        <button type="submit" /*onClick={makeRequest}*/>Generate</button>
                    </div> :
                    <div>
                        <Name onChange={setNameFunction_P}/>
                        <Framework frameworks={{fw1: "unittest", titleFw1: "unittest", fw2: "pytest", titleFw2: "pytest"}}
                        onChange={setFrameworkFunction_P}/>
                        <LibraryPattern sendArray={setArrayFunctionLibrary_P} isLangChanged={isLangChanged}
                                        data={dataLibrary.python} header={{header: "Add library"}}/>
                        <LibraryPattern sendArray={setArrayFunctionPattern_P} isLangChanged={isLangChanged}
                                        data={dataPattern.python} header={{header: "Add pattern"}}/>
                        <button type="submit" /*onClick={makeRequest}*/>Generate</button>
                    </div>
                }
            </form>
